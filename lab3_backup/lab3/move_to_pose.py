...
class MoveToPoseServer(Node):
    ...
    async def execute_callback(self, goal_handle):
        ...
        # manually start the first iteration
        self.handle_odom(Odometry())
    ...
    def handle_odom(self, odometry: Odometry):
        ...
        if self.goal_handle_ == None:
            ...
            self.get_logger().info("ararar")
            return

        self.get_logger().info("handling odometry")
        ...
        # now that we have linear and angular, move robot
        self.cmd_publisher_.publish(self.cmd_vel_)

        self.get_logger().info("waiting for PID calculation")
        if linear_error < self.threshold_ and angular_error < self.threshold_:
            ...
            self.get_logger().info("calculating and sending feedback")
